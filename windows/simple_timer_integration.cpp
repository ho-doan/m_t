...
// Simple Timer Integration for Flutter App
// This can be called from Flutter plugin

std::wstring g_pipeName = L"\\\\.\\pipe\\flutter_timer_pipe";
...
void RunParent() {
    ...
    wchar_t cmdLine[] = L"simple_timer_integration.exe child";
...
extern "C" {
    __declspec(dllexport) void StartFlutterTimer() {
        log(L"Flutter: Starting timer");
        RunParent();
    }
    
    __declspec(dllexport) void StopFlutterTimer() {
        log(L"Flutter: Stopping timer");
        g_timerRunning.store(false);
    }
    
    __declspec(dllexport) bool IsTimerRunning() {
